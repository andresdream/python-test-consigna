...
resource "aws_route_table" "vpc_public_subnet3_routing_table" {
  vpc_id = aws_vpc.main.id
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.vpc_ig.id
  ...
  tags = {
      Name = "${var.layer}-${var.stack_id}-public_subnet3_routing_table"
  }
}
...
resource "aws_route_table" "vpc_private_subnet3_routing_table" {
  vpc_id = aws_vpc.main.id
  route {
    cidr_block = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.vpc_nat_gw.id
  ...
  tags = {
      Name = "${var.layer}-${var.stack_id}-vpc_private_subnet3_routing_table"
  }
}
...
resource "aws_route_table_association" "vpc_public_subnet3_routing_table_association" {
  subnet_id      = aws_subnet.public3.id
  route_table_id = aws_route_table.vpc_public_subnet3_routing_table.id
}
